...
// console.log(data[0].orgs);
  ...
  svg1.selectAll(".bar")
      ...
      .on("mousemove", function(d) {
        var xPosition = d3.mouse(d3.select('.svg1').node())[0] - 20;
        var yPosition = d3.mouse(d3.select('.svg1').node())[1] - 20;
...
d3.csv("data2.csv", function(error, data) {
  ...
  region.selectAll(".bar")
      ...
      .on("mousemove", function(d) {
        ...
        tooltip2.select("text").text(d.y1-d.y0);
      });
...
d3.csv("data3.csv", function(error, data) {
  ...
  state.selectAll(".bar")
      ...
      .on("mousemove", function(d) {
        ...
        tooltip3.select("text").text(d.y1-d.y0);
      });
...
// // Prep the tooltips, initial display is hidden
      var tooltip1 = d3.select(".svg1").append("g")
        .attr("class", "tooltip")
        .style("display", "none");
